...
func open_file(path: String, data: DialogueData = null) -> void:
	var file_name: String = path.split('/')[-1]
...
func switch_file(idx: int, ensure_path := '') -> void:
	...
	if cur_idx > -1:
		...
		if workspace.has_node('Graph'):
			editor.add_menu.get_popup().id_pressed.disconnect(
...
func _on_empty_clicked(at_pos: Vector2, mouse_button_index: int) -> void:
	if mouse_button_index == MOUSE_BUTTON_RIGHT:
		...
		if not EditorInterface.get_editor_settings().get_setting(
