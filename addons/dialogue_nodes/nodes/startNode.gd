...
func data_to_tree(graph: GraphEdit, data: DialogueData, node_name := name) -> void:
	...
	if node_name == name:
		...
		if data.nodes[node_name].has('type'):
			type = data.nodes[node_name]['type']
		else:
			# Existing DialogueNodes files use the numeric node ID
			# embedded in the node name.
			type = int(node_name.split('_')[0])
