...
func set_data(new_data: DialogueData) -> void:
	...
	if data.characters.ends_with('.tres'):
		var character_list = ResourceLoader.load(
			data.characters,
			'',
...
func _proceed(node_name: String) -> void:
	...
	if node_data.has('type'):
		...
		if node_type.begins_with('custom:'):
			_process_custom(node_type.trim_prefix('custom:'), node_data)
			return
...
func _process_dialogue(dict: Dictionary) -> void:
	...
	for option in dict.options.values():
		if (
			option.condition.is_empty() or _check_condition(option.condition)
			or skip_options_condition_checks
		):
...
func _process_custom(id: String, node_data: Dictionary) -> void:
	var processor := DialogueNodes.get_processor(StringName(id))

	if not processor.is_valid():
		push_error("DialogueParser: No processor registered for custom node '%s'." % id)
		return

	processor.call(node_data, self)
...
func _update_wait_tags(node: RichTextLabel, value: String) -> String:
		...
		match value[idx]:
			'[':
				...
				if open_tag_start == idx:
					...
					var insert_text := ' start=' + str(start_data.start) + ' last=' + str(
						start_data.last
	...
	while len(waits) > 0:
		var start_data: Dictionary = waits.pop_back()
		var insert_text := ' start=' + str(start_data.start) + ' last=' + str(char_count - 1) + ' length=' + str(
			text_length
		)
		value = value.insert(start_data.at, insert_text)
